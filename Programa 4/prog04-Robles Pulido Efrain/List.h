...
class List {
    ...
    public:
        ...
        std::string getAllList(Position p);//Regresara en foma de cadena el formato para la tabla de indices
...
std::string List<T>::toString() const {
    ...
    while(aux != nullptr) {//Iteracion donde imprimira toda la lista con su respectivo formato
...
template <class T>
std::string List<T>::getAllList(Position p) {//Regresara en foma de cadena el formato para la tabla de indices
    string myStr;
    Position aux(p);
    ...
    while(aux != nullptr) {//Iteracion donde imprimira toda la lista con su respectivo formato
        ...
        myStr += to_string(info.getNumber())+'|'+info.getStreet()+'|'+info.getColony()+'*';//Escribira para el archivo de indices
        aux = aux->getNext();
        }
