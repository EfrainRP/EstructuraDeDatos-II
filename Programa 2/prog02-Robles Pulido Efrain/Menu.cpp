        ...
        switch (myCharA) {
            case 'C':///Busqueda por numero de casa
                ...
                if((pointSearch = myBTree->findData(element)) != nullptr) {
                    element = myBTree->retrieve(pointSearch);//Metodo que nos recuperara el elemento del arbol a partir de la posicion
...
void Menu::showParseOrder() {//Metodo para mostrar todos los datos del arbol segun el recorrido deseado
    ...
    do {
        system("cls");//Limpia toda la pantalla
        cout<<"\n\t[1] PreOrder\n\t[2] InOrder\n\t[3] PostOrder\n\t[4] Con forma de arbol (grafica en horizontal)\n\t[5] Regresar\n"
        << "Seleccione forma de recorrido del Arbol: ";

        cin>>opc;

        if(opc=='4'){
            cout << endl <<"(Grafica en forma de arbol con sentido horizontal hacia la izquierda)"<< endl << endl << endl;
            myBTree->showLikeTree();
        }
        ...
        if(opc>53) { //Valores de la tabla ascii mayores del '5'
...
void Menu::showInfo() {//Metodo de la interfaz para SOLO conocer los elementos dentro del arbol
    ...
    cout<<"Tabla de elementos del arbol en un recorrido pre-Order para visualizar los elementos"<<endl<<endl;
